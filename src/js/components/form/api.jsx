...
export default () => <div>
  ...
  <ul className="simple-list">
    ...
    is suppressed.</li>
    <li>If an async valdation is initiated, the previously initiated in progress async validation
    is suppressed.</li>
  ...
  <ul className="simple-list">
    ...
    <li>Passes a <Ticks text="{input, event, target}" /> object to func.</li>
    <li>If there is an existing error message, it returns it until the input becomes
    valid. The error message can be replaced by subsequent validations.</li>
    <li>If the validation function reports an error, it is returned
    only in one of the following cases:
      <ul>
        <li>ignoreEvent is true.</li>
        <li>the validation event is included in the list of events and the
        target matches. If the target function is passed, the validation target must match
        the result of calling this function. If the target function is not passed, the validation
        target must match the input itself. If the target function is passed, it receives
        a <Ticks text="{input, event, target}" /> object.</li>
        <li>the event is submit (even if 'submit' is not included in the
        list of events).</li>
      </ul>
    </li>
    <li>The second object argument is optional.</li>
    <li>The default events list consists only of 'blur'.</li>
    <li>The default events can be changed by setting validate.events, which can be overwritten
    by passing an events option.</li>
    <li>Creates a closure that keeps internal state and therefore the resulted function
    can not be reused for multiple inputs. However, the function argument can be reused.</li>
  </ul>

  <Api id="required" text={`
  ...
  <ul className="simple-list">
    ...
    it returns null.</li>
    <li>It's based on the <Ticks text="validate()" /> functionality, so it can rely on its defaults.</li>
    <li>The default error message is 'Required'. This can be overwritten by
    setting a required.message default function, which can be
    overwritten by passing the message option. The message function receives a <Ticks
    text="{value, input, event, target}" /> object argument, where value is the input's current value.</li>
    <li>The default events can be changed by setting required.events, which can be overwritten
    by passing an events option.</li>
    <li>ignoreEvent is passed to <Ticks text="validate()" />.</li>
    <li>Creates a closure that keeps internal state and therefore the resulted function
  ...
  <ul className="simple-list">
    <li>The second object argument is optional.</li>
    <li>If the input value is a number less than minValue returns an error message.</li>
    <li>It's based on the <Ticks text="validate()" /> functionality, so it can rely on its defaults.</li>
    <li>The default error message is `Minimum allowed is {'${minValue}'}`.
    This can be overwritten by
    setting a min.message default function, which can be
    overwritten by passing the message option. The message function receives a <Ticks
    text="{minValue, value, input, event, target}" /> object argument, where the minValue is the
    minValue passed to min and the value is the input's current value.</li>
    <li>The default events can be changed by setting min.events, which can be overwritten
    by passing an events option.</li>
    <li>ignoreEvent is passed to <Ticks text="validate()" />.</li>
    <li>Creates a closure that keeps internal state and therefore the resulted function
    can not be reused for multiple inputs.</li>
  ...
  <ul className="simple-list">
    <li>The second object argument is optional.</li>
    <li>If the input value is a number greater than maxValue returns an error message.</li>
    <li>It's based on the <Ticks text="validate()" /> functionality, so it can rely on its defaults.</li>
    <li>The default error message is `Maximum allowed is {'${maxValue}'}`.
    This can be overwritten by
    setting a max.message default function, which can be
    overwritten by passing the message option. The message function receives a <Ticks
    text="{maxValue, value, input, event, target}" /> object argument, where the maxValue
    is the maxValue passed to max and the value is the input's current value.</li>
    <li>The default events can be changed by setting max.events, which can be overwritten
    by passing an events option.</li>
    <li>ignoreEvent is passed to <Ticks text="validate()" />.</li>
    <li>Creates a closure that keeps internal state and therefore the resulted function
    can not be reused for multiple inputs.</li>
  ...
  <ul className="simple-list">
    ...
    minLengthValue returns an error message.</li>
    <li>It's based on the <Ticks text="validate()" /> functionality, so it can rely on its defaults.</li>
    <li>The default error message is `Minimum allowed length is {'${minLengthValue}'}`.
    This can be overwritten by
    setting a minLength.message default function, which can be
    overwritten by passing the message option. The message function receives a <Ticks
    text="{minLength, value, input, event, target}" /> object argument, where the minLength is the
    minLengthValue passed to minLength and the value is the input's current value.</li>
    <li>The default events can be changed by setting minLength.events, which can be overwritten
    by passing an events option.</li>
    <li>ignoreEvent is passed to <Ticks text="validate()" />.</li>
    <li>Creates a closure that keeps internal state and therefore the resulted function
    can not be reused for multiple inputs.</li>
  ...
  <ul className="simple-list">
    ...
    maxLengthValue returns an error message.</li>
    <li>It's based on the <Ticks text="validate()" /> functionality, so it can rely on its defaults.</li>
    <li>The default error message is `Maximum allowed length is {'${maxLengthValue}'}`.
    This can be overwritten by
    setting a maxLength.message default function, which can be
    overwritten by passing the message option. The message function receives a <Ticks
    text="{maxLength, value, input, event, target}" /> object argument, where the maxLength is the
    maxLengthValue passed to maxLength and the value is the input's current value.</li>
    <li>The default events can be changed by setting maxLength.events, which can be overwritten
    by passing an events option.</li>
    <li>ignoreEvent is passed to <Ticks text="validate()" />.</li>
    <li>Creates a closure that keeps internal state and therefore the resulted function
    can not be reused for multiple inputs.</li>
  ...
  <ul className="simple-list">
    <li>promiseFunc must return a promise or a falsy value.</li>
    <li>The second object argument is optional.</li>
    ...
    <li>If promiseFunc returns a falsy value it's as if promiseFunc wasn't called at all.</li>
    <li>Creates a closure that keeps internal state and therefore the resulted function
    can not be reused for multiple inputs. However, the function argument can be reused.</li>
  </ul>
</div>;
